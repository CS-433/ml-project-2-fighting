...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    with tf.Session() as s:
        ...
        test_img = [mpimg.imread(test_dir + 'test_' + str(i + 1) + '/' + 'test_' + str(i + 1) + '.png') for i in range(n)]
        results = []
        for i in range(n):
            test_prediction = get_prediction(test_img[i])
            results.append(test_prediction)
        output_dir = "output/"
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        save_result(output_dir + 'submission.csv', results)
